namespace Spellslinger.Misc
{
    ...
    public class SaveRunes3D : MonoBehaviour {
        ...
        private const string FILEPATH = "3DTime.txt";
        ...
        private void SaveRuneToFile() {
            ...
            for (int i = 0; i < this.lineRenderer.positionCount; i++) {
                globalPoints.Add(this.lineRenderer.GetPosition(i));
                Debug.Log("Recorded Points " + i + ": " + this.lineRenderer.GetPosition(i));
            }
            ...
            if (savedPoints.Length == MAX_POINTS) {
                ...
                foreach (Vector3 point in savedPoints) {
                    line += point.x + "," + point.y + "," + point.z + ";"; // dataset shape
                    //line += n + "X=" + point.x + "\t\t\tY=" + point.y + "\t\t\tZ=" + point.z + ";\n";
                    n++;
                }
            ...
            for (int i = 0; i < savedPoints.Length; i++) {
                Debug.Log("Normalized Points " + i + ": " + savedPoints[i]);
        ...
        private Vector3[] GenerateNormalizedList(List<Vector3> originalPoints, Transform transform) {
            ...
            for (int i = 0; i < normalizedPoints.Count; i++) {
                ...
                // keep it in local space
                //cameraPoints[i] = /*(Camera.main.WorldToScreenPoint(transform.TransformPoint(*/normalizedPoints[i]/*))*/;

                // rotate the localpoints around the y-axis. This is necessary to make sure that the output points are always in the same direction.
                
                //cameraPoints[i] = transform.InverseTransformPoint(normalizedPoints[i]); //move into lcocal space
                //rotate around local camera axis
                //cameraPoints[i] = RotatePointAroundPivot(cameraPoints[i], Camera.main.transform.position, Camera.main.transform.localEulerAngles);

                //draw it straight ahead lol fuck it.

                cameraPoints[i] = normalizedPoints[i];
                Debug.Log("Rotated for Camera" + i + ": " + cameraPoints[i] + "EULER ANGLES: " + Camera.main.transform.localEulerAngles);
            }
        ...
        private Vector3[] NormalizeList(Vector3[] standarizedPositions) {
            ...
            foreach (Vector3 pos in standarizedPositions) {
                Debug.Log("Unstandardized Points: " + pos);
                if (pos.x < smallestX) {
                    smallestX = pos.x;
                }
            ...
            for (int i = 0; i < standarizedPositions.Length; i++) {
                /*
                standarizedPositions[i].x /= biggestComponentX;
                standarizedPositions[i].y /= biggestComponentY;
                standarizedPositions[i].z /= biggestComponentZ;
                */
